...
# Called after each individual run completes, with (result, runs_done, total_runs).
OnRunComplete = Callable[[SimulationResult, int, int], None]
...
def run_experiment(
    ...
    config: SimulationConfig = DEFAULT_CONFIG,
    progress: bool = True,
    on_run_complete: Optional[OnRunComplete] = None,
) -> Path:
    """Runs all 180 combinations and writes the complete output directory.
    Returns the timestamped output directory path.

    `on_run_complete`, if given, is called synchronously after every single
    run finishes (result, runs_done, total_runs) -- this is the hook the web
    UI (see web/backend) uses to stream genuine live progress without
    re-implementing this loop. It has no effect on CLI behavior (default
    None) and every value it receives is the same real `SimulationResult`
    this function already produces -- no duplicate computation.
    """
        ...
        for seed in seeds:
            ...
            for strategy_name, factory in STRATEGY_FACTORIES.items():
                ...
                if on_run_complete is not None:
                    on_run_complete(result, done, TOTAL_RUNS)
