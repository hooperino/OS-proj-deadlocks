...
config=DEFAULT_CONFIG, progress=True) -> Path` -- the full 180-run
comparison; returns the timestamped output directory
(`<output_root>/experiment_<YYYYMMDD_HHMMSS>/`) containing `raw/`,
`raw_all.json`, `aggregate.csv`, `summary.csv`, `run_report.txt`, `plots/`.
`TOTAL_RUNS` (180) and `STRATEGY_FACTORIES` are importable constants other
modules (tests, CLI) rely on rather than recomputing.
